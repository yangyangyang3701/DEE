...
def extract_doc_valid_span_info(doc_token_type_mat, doc_fea):
    # extract span from predicted/gold token type labels
    ...
    for sent_idx in range(valid_sent_num):
        ...
        while char_s < seq_len:
            # if token is pad, then it means the sentence has come to an end
            if seq_token_mask_list[char_s] == 0:
                break

            entity_idx = seq_token_type_list[char_s]

            if entity_idx % 2 == 1:  # if entity is start with `B-`
                char_e = char_s + 1
                # when former entity label is started with `I-`
                while (
                    char_e < seq_len
...
def get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=False):
    ...
    for doc_token_types, doc_fea in zip(doc_token_types_list, doc_fea_list):
        ...
        if use_gold_span:
            ...
            """
            DONE: check the availability to use gold_span while evaluating
        ...
        # just flatten the dranges and mentions from sentence-independent data orgnisation format to flat list format
        (
            span_mention_range_list,
            mention_drange_list,
            mention_type_list,
...
def get_laap_doc_span_info_list(
