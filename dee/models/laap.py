...
from dee.modules.doc_info import get_laap_doc_span_info_list
from dee.modules.event_table import EventTable
from dee.modules.ner_model import NERModel, LSTMCRFNERModel


class HungarianMatcher(nn.Module):
    """
    This class computes an assignment between the targets and the predictions of the network.
    For efficiency, the targets don't include 'no_object', so there are generally more predictions than targets.
    In such cases, a 1-to-1 matching of the best predictions is performed, while unmatched predictions are treated as non-objects.
    """
    ...
    def __init__(self, cost_weight, matcher, num_event_type):
        ...
        Args:
            cost_weight (dict): Weights for event type and role matching.
            matcher (str): The matching strategy to use ('avg' in this case).
            num_event_type (int): Number of event types.
        """
        super().__init__()
    ...
    def forward(self, outputs, targets):
        """
        Computes the optimal matching between predictions and targets.

        Args:
            outputs (dict): Model predictions, including predicted event types and roles.
            targets (dict): Ground truth labels for event types and roles.

        Returns:
            torch.Tensor: Indices representing the optimal assignment.
        """
        # Get dimensions for role logits
        num_sets, num_roles, num_entities = outputs["pred_role_logits"].size()

        # Calculate the softmax of predicted event types and roles
        pred_event = outputs["pred_doc_event_logps"].softmax(-1)  # [num_sets, num_event_types]
        gold_event = targets["doc_event_label"]

        # Convert gold event labels to tensor and handle specific cases
        gold_event_tensor = torch.tensor(gold_event).cuda()
        if self.num_event_type == 2:
            gold_event_tensor = torch.zeros_like(gold_event_tensor).long().cuda()

        pred_role = outputs["pred_role_logits"].softmax(-1)  # [num_sets, num_roles, num_entities]
        gold_role = targets["role_label"]

        # Filter out None roles from gold_role
        gold_role_lists = [role_list for role_list in gold_role if role_list is not None]
        gold_role = torch.tensor(gold_role_lists).cuda()

        # Split the predictions and targets into lists for pairwise comparison
        pred_role_list = pred_role.split(1, 1)
        gold_role_list = gold_role.split(1, 1)

        # Calculate costs based on the specified matcher strategy
        if self.matcher == "avg":
            cost_list = []
        ...
        # Stack and reshape role cost tensors for final cost calculation
        role_cost_tensor = torch.stack(cost_list).transpose(1, 0).view(num_sets, num_roles, -1)
...
class SetCriterion(nn.Module):
    ...
    The process occurs in two steps:
        1) Compute Hungarian assignment between ground truth and model outputs.
        2) Supervise each matched ground-truth/prediction pair (class, subject position, and object position).
    """

    def __init__(self, config, num_classes, event_type_weight=False, cost_weight=False, na_coef=0.1,
                 losses=["event", "role"], matcher="avg"):
        """
        Initializes the SetCriterion.

        Args:
            config (object): Configuration object.
            num_classes (int): Number of event types.
            event_type_weight (bool or list): Weights for event types, if provided.
            cost_weight (dict): Weights for event type and role matching.
            na_coef (float): Coefficient applied to the NA category classification weight.
            losses (list): List of losses to be applied.
            matcher (str): The matching strategy ('avg' in this case).
        """
        super().__init__()
        ...
        if event_type_weight:
            ...
            self.type_weight[-1] = na_coef  # Apply NA coefficient to the last class
    ...
    def forward(self, outputs, targets):
        ...
        Args:
            outputs (dict): Model predictions, including predicted event types and roles.
            targets (list of dicts): Ground truth labels for event types and roles.

        Returns:
            torch.Tensor: Computed loss.
        """
        # Retrieve the optimal matching between predictions and targets
        indices_tensor = self.matcher(outputs, targets)

        # Compute the role loss and event type loss
        losses = self.get_role_loss(outputs, targets, indices_tensor)
        return losses

    def get_role_loss(self, outputs, targets, indices_tensor):
        """
        Computes the role and event type loss.

        Args:
            outputs (dict): Model predictions.
            targets (dict): Ground truth labels.
            indices_tensor (torch.Tensor): Optimal assignment indices.
        ...
        # Calculate softmax for predicted event types and roles
        pred_event = outputs["pred_doc_event_logps"].softmax(-1)
        gold_event = targets["doc_event_label"]
        gold_event_tensor = torch.tensor(gold_event).cuda()

        pred_role = outputs["pred_role_logits"].softmax(-1)
        gold_role = targets["role_label"]
        gold_role_tensor = torch.tensor(gold_role).cuda()

        # Handle specific case where there are only two classes
        if self.num_classes == 2:
            gold_event_tensor = torch.zeros_like(gold_event_tensor).long().cuda()

        # Select the matched predictions and targets based on optimal assignment
        selected_pred_role_tensor = pred_role[indices_tensor[0]]
        selected_gold_role_tensor = gold_role_tensor[indices_tensor[1]]

        # Compute cross-entropy loss for key roles
        key_role_weight = torch.ones(num_entities).cuda()
        key_role_weight[-1] = 1  # Full weight for key roles
        ...
        # Compute cross-entropy loss for other roles with reduced weight
        rest_role_weight = torch.ones(num_entities).cuda()
        rest_role_weight[-1] = 0.2  # Reduced weight for non-key roles
        ...
        # Compute cross-entropy loss for event type classification
        ...
        # Handle training for non-paired sets if enabled in the config
        if self.config.laap_train_nopair_sets:
...
class SetPred4DEE(nn.Module):
    def __init__(self, config, event_type2role_index_list, return_intermediate=True):
        ...
        self.config = config
        event_type_classes = config.laap_event_type_classes
        self.num_generated_sets = config.laap_num_generated_sets
        num_set_layers = config.laap_num_set_decoder_layers
        num_role_layers = config.laap_num_role_decoder_layers
        self.cost_weight = config.laap_cost_weight
        ...
        if config.laap_use_event_type_enc:
            self.event_type_embed = nn.Embedding(
                len(event_type2role_index_list), config.hidden_size
            )

        self.return_intermediate = return_intermediate
        self.dropout = nn.Dropout(config.laap_hidden_dropout)
        self.LayerNorm = nn.LayerNorm(
            config.hidden_size, eps=config.laap_layer_norm_eps
        )
        if num_set_layers > 0:
        ...
        self.event_type_weight = config.laap_event_type_weight
        ...
        if self.config.laap_use_sent_span_encoder:
        ...
        if self.config.laap_use_role_decoder:
            self.event2role_decoder = transformer.make_transformer_decoder(
                config.laap_num_event2role_decoder_layer,
                config.hidden_size,
                ff_size=config.ff_size,
                dropout=config.dropout,
            )
    ...
    def forward(
        ...
        if self.config.laap_use_event_type_enc:
...
class LAAPModel(nn.Module):
    def __init__(self, config, event_type_fields_pairs, ner_model=None):
        ...
        if config.laap_train_on_multi_roles is True:
    ...
    def get_mix_loss(self, doc_sent_loss_list, doc_event_loss_list, doc_span_info_list):
        ...
        lambda_1 = self.config.laap_ner_loss_weight
        lambda_2 = self.config.laap_type_loss_weight
        lambda_3 = self.config.laap_event_generation_loss_weight
    ...
    def get_local_context_info(
        ...
        # get LSTMCRFNERModel output
        ner_token_emb, ner_loss, ner_token_preds, _ = self.ner_model(
            ner_token_ids,
            ner_token_masks,
    ...
    def forward(
        ...
        doc_span_info_list = get_laap_doc_span_info_list(
