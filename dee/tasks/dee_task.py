...
class DEETaskSetting(TaskSetting):
    ...
    base_attr_default_pairs = [
        ...
        ("laap_ner_loss_weight", 0.1),  # the proportion of ner loss
        (
            "laap_type_loss_weight",
            0.4,
        ),  # the proportion of event type classification loss
        (
            "laap_event_generation_loss_weight",
            0.5,
        ),  # the proportion of event generation loss
        ("laap_decoder_lr", 2e-5),  # learning rate of LAAP decoder
        ("laap_num_event2role_decoder_layer", 4),
        ("laap_train_on_multi_events", True),
        ("laap_train_on_single_event", True),
        ("laap_event_type_classes", 2),
        ("laap_num_generated_sets", 5),
        ("laap_num_set_decoder_layers", 2),
        ("laap_num_role_decoder_layers", 4),
        ("laap_cost_weight", {"event_type": 1, "role": 0.5}),
        ("laap_train_on_multi_roles", False),
        ("laap_use_event_type_enc", True),
        ("laap_use_role_decoder", True),
        ("laap_use_sent_span_encoder", False),
        ("laap_train_nopair_sets", True),
        ("laap_hidden_dropout", 0.1),
        ("laap_layer_norm_eps", 1e-12),
        ("laap_event_type_weight", [1, 0.2]),
...
class DEETask(BasePytorchTask):
    ...
    def __init__(
        ...
        if self.setting.model_type == "LAAPModel":
            bert_config = BertConfig.from_pretrained(self.setting.bert_model)
            self.setting.update_by_dict(bert_config.__dict__)
            self.setting.model_type = "LAAPModel"
        ...
        if self.setting.model_type == "LAAPModel":
            ...
            optimizer_grouped_parameters = [
                ...
                },
                {
                    ...
                    "weight_decay_rate": 0.01,
                    "lr": self.setting.laap_decoder_lr,
                },
                {
                    "params": [
                        p
        ...
        elif self.setting.model_type == "LAAPModel":
            # use DEEFeature
            convert_dataset_func = convert_laap_features_to_dataset
            self.feature_converter_func = LAAPFeatureConverter(
                self.entity_label_list,
                self.event_template,
                self.setting.max_sent_len,
                self.setting.max_sent_num,
    ...
    def get_loss_on_batch(self, doc_batch_dict, features=None):
        ...
        try:
            ...
            # DONE(): fix this issue for multi-gpu DDP training
            logger.info("-" * 30)
            logger.info(
