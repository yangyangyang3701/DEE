...
class LAAPFeatureConverter(object):
    def __init__(
        self,
        entity_label_list,
        template,
...
def convert_laap_features_to_dataset(dee_features):
    # just view a list of doc_fea as the dataset, that only requires __len__, __getitem__
    assert len(dee_features) > 0 and isinstance(dee_features[0], LAAPFeature)
